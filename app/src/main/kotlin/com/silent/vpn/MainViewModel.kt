...
class MainViewModel @Inject constructor(
    ...
    private var paymentInitJob: Job? = null
    ...
    private val _subscriptionProbeActive = MutableStateFlow(false)
    val subscriptionProbeActive: StateFlow<Boolean> = _subscriptionProbeActive
    ...
    /** 0..1 прогресс стартового bootstrap на splash (подписка + WG-кеш). */
    private val _launchBootstrapProgress = MutableStateFlow(0f)
    val launchBootstrapProgress: StateFlow<Float> = _launchBootstrapProgress

    private val _launchBootstrapActive = MutableStateFlow(false)
    val launchBootstrapActive: StateFlow<Boolean> = _launchBootstrapActive

    private fun setLaunchBootstrapProgress(value: Float) {
        val next = value.coerceIn(0f, 1f)
        if (next >= _launchBootstrapProgress.value) {
            _launchBootstrapProgress.value = next
        }
    }
    ...
    @Volatile private var awaitingGetconfAccess = false
    /** GETCONF выдал WG — доступ с сервера есть. Stale /me и кеш «нет» эту сессию не гасят. */
    @Volatile private var getconfGrantedThisVpnSession = false
    ...
    private val connectPrefetchMutex = Mutex()
    private var launchPrefetchJob: Job? = null
    private val prefetchedConnectBySlot = mutableMapOf<String, ConnectFetchResult>()
    private var pendingGetconfProfile: UserProfile? = null
    @Volatile private var lastVpnConfigDenied = false
        ...
        override fun onHashesUpdated(items: List<HashItemDto>, applyToTunnel: Boolean) {
            ...
            if (applyToTunnel) {
                ...
                if (serverHashes.isNotEmpty()) {
                    val stateNow = _vpnState.value
                    val tunnelAgeMs = if (tunnelUpAtMs > 0L) System.currentTimeMillis() - tunnelUpAtMs else -1L
                    val skipHotApply =
                        stateNow == VpnState.CONNECTING ||
                            (
                                stateNow == VpnState.CONNECTED &&
                                    (tunnelAgeMs < 0L || tunnelAgeMs < HASH_HOT_APPLY_GRACE_MS)
                                )
                    if (skipHotApply) {
                        DebugLog.i(
                            "MainViewModel",
                            "config sync hash apply deferred: state=$stateNow tunnelAge=${tunnelAgeMs}ms",
                        )
                        return
                    }
            ...
            WdttTunnelManager.lastError.collect { err ->
                if (!err.isNullOrBlank() &&
                    ...
                    if (paymentBootstrapHeld) {
                        ...
                        getconfGrantedThisVpnSession = false
    ...
    private suspend fun refreshSession() {
        ...
        // WG-конфиг и подписка на LTE — prefetchConnectConfigAtLaunch на splash,
        // не второй bootstrap (он мигал тумблер при включении VPN).
    }
    ...
    private suspend fun needsPaymentInternetBridge(): Boolean {
        ...
        val bootstrap = paymentBootstrapHeld || WdttTunnelManager.isBootstrapMode()
    ...
    private fun applyCachedProfileAfterSync() {
        if (awaitingGetconfAccess) {
            DebugLog.i("MainViewModel", "skip cache profile — waiting GETCONF")
            return
        }
        val cached = repo.getCachedProfile() ?: return
        if (shouldIgnoreInactiveAfterGetconf(cached)) {
            DebugLog.i("MainViewModel", "skip unpaid cache — GETCONF already granted")
            return
        }
        applyServerProfile(cached, force = true)
    }
     ...
     * На старте приложения (splash) и по действию пользователя; не фоновый poll.
     */
    private suspend fun runEphemeralApiBootstrap(
        ...
        if (_vpnState.value == VpnState.CONNECTED) return false
        // CONNECTING без main VPN — тумблер ждёт стартовый prefetch. Ephemeral старта должен дописать WG-кеш.
        if (
            _vpnState.value == VpnState.CONNECTING &&
            SilentVpnService.isRunning &&
            !WdttTunnelManager.isBootstrapMode()
        ...
        val maxAttempts = if (forLaunch) EPHEMERAL_LAUNCH_WAIT_ITER else EPHEMERAL_TUNNEL_WAIT_ITER

        silentBootstrapSync = true
        try {
            repo.clearTunnelApiBase()
            // LTE: публичный bootstrapConfig почти всегда таймаут — сразу локальный boot-хеш.
            ...
            DebugLog.i("MainViewModel", "ephemeral API bootstrap start mobile=$onMobile launch=$forLaunch")
            if (forLaunch) setLaunchBootstrapProgress(0.52f)
            launchVpnService(context.applicationContext, config, forceBootstrap = true)
            var apiOk = false
            var attempt = 0
            while (attempt < maxAttempts && !apiOk) {
                if (ephemeralShouldYieldToUser()) {
                    DebugLog.i("MainViewModel", "ephemeral API bootstrap: user took over VPN")
                    break
                }
                delay(if (forLaunch) 200 else 250)
                if (!WdttTunnelManager.tunnelReady.value || !WdttTunnelManager.isBootstrapMode()) {
                    attempt++
                    if (forLaunch && attempt % 8 == 0) {
                        setLaunchBootstrapProgress(0.52f + (attempt.toFloat() / maxAttempts) * 0.28f)
                    }
                    continue
                }
                if (
                ...
                if (attempt == 8 || attempt == 16) delay(if (forLaunch) 400 else 800)
                if (forLaunch) setLaunchBootstrapProgress(0.78f)
                val ok = runCatching {
                ...
                if (ok) {
                    ...
                    if (forLaunch) setLaunchBootstrapProgress(0.92f)
            ...
            val userTookOver = ephemeralShouldYieldToUser()
            silentBootstrapSync = false
            // A payment can acquire the existing bootstrap while API sync is finishing.
            if (!userTookOver && !paymentBootstrapHeld && WdttTunnelManager.isBootstrapMode()) {
                stopVpnLocally(context.applicationContext)
                repo.clearTunnelApiBase()
                if (forLaunch) {
                    waitVpnServiceDown()
    ...
    private suspend fun ensurePaymentBootstrapHeldLocked(context: Context, force: Boolean = false): Boolean {
        ...
        launchVpnService(context.applicationContext, config, forceBootstrap = true)
        if (awaitPaymentApiReady()) return true
        DebugLog.w("MainViewModel", "payment bootstrap: tunnel API not ready")
        stopPaymentBootstrap(context)
        waitVpnServiceDown()
        return false
    }

    /** Быстро: иконка VPN + API для initPayment. Маршруты браузера — отдельно перед Chrome. */
    private suspend fun awaitPaymentApiReady(): Boolean {
        repeat(80) { attempt ->
            ...
            if (
                ...
                repo.ensureBootstrapTunnelApi()
                DebugLog.i("MainViewModel", "payment API tunnel ready (attempt ${attempt + 1})")
                return true
            }
            delay(100L)
        }
    ...
    private suspend fun awaitPaymentBrowserRoutes(): Boolean {
        // TURN обычно появляется за секунды; не ждём worker-stats (~10с).
        val maxAttempts = 100 // ~10s @ 100ms
        repeat(maxAttempts) { attempt ->
            if (!paymentBootstrapHeld) return false
            val ready = try {
                WdttTunnelManager.prepareBootstrapInternetForPayment()
            } catch (e: CancellationException) {
                throw e
            } catch (e: Exception) {
                DebugLog.w("MainViewModel", "payment routes: ${e.message}")
                false
            }
            if (ready) {
                repo.ensureBootstrapTunnelApi()
                DebugLog.i("MainViewModel", "payment browser ready (attempt ${attempt + 1})")
                return true
            }
            delay(100L)
        }
    ...
    private suspend fun <T> withEphemeralBackendApi(block: suspend () -> T): T {
        ...
        if (!waitUntilTunnelHttpReady()) {
            error("tunnel api not ready")
        }
    ...
    /** tunnelReady ещё не значит, что TCP к 10.66.66.1:8000 уже идёт через WG (первый SYN с LTE таймаутится 12с). */
    private suspend fun waitUntilTunnelHttpReady(): Boolean {
        repeat(15) {
            val ok = runCatching {
                if (SilentRepository.APP_EXCLUDED_FROM_VPN) {
                    WdttTunnelManager.withApiOverlay {
                        repo.probeTunnelGateway()
                    }
                } else {
    ...
    private suspend fun fetchVpnConfigForConnect(
        context: Context,
        fp: String,
        allowEphemeral: Boolean = true,
        forLaunch: Boolean = false,
        preferredServer: String = repo.getPreferredServer(),
        ...
        if (!accessDenied && allowEphemeral && vpnConfig == null && publicFailed) {
            if (runEphemeralApiBootstrap(context, force = true, forLaunch = forLaunch)) {
                ...
                } else if (lastVpnConfigDenied) {
                    accessDenied = true
                    applySubscriptionDeniedFromApi()
                    apiError = subscriptionRequiredMessage()
                } else {
                    val cached = loadCachedVpnConfig()
                    vpnConfig = cached?.takeIf { isConfigConnectable(it) && cachedConfigMatchesPreferred(it, targetSlot) }
                    if (vpnConfig != null) {
                        repo.setVpnAccessDenied(false)
                        repo.mergeSavedHashesIntoCachedConfig()
                        vpnConfig = loadCachedVpnConfig()?.takeIf {
                            isConfigConnectable(it) && cachedConfigMatchesPreferred(it, targetSlot)
                        }
                    }
                }
            }
        ...
        if (vpnConfig == null && !accessDenied && forLaunch && !onMobile) {
            runCatching {
                repo.clearTunnelApiBase()
                repo.useApiBase(repo.getPublicServerUrl())
                repo.invalidateApiClient()
                ...
                if (cfgRes.isSuccessful) {
                    ...
                    if (candidate != null && isConfigConnectable(candidate) &&
                        cachedConfigMatchesPreferred(candidate, targetSlot)
                    ) {
                        vpnConfig = candidate
                        if (persistToRepoCache) {
                            repo.cacheVpnConfig(Gson().toJson(candidate))
                ...
                DebugLog.w("MainViewModel", "launch public fallback: ${it.message}")
            }
        }
        if (vpnConfig == null && !accessDenied && hasVpnAccess() && targetSlot == repo.getPreferredServer()) {
            vpnConfig = loadCachedVpnConfig()?.takeIf { isConfigConnectable(it) && cachedConfigMatchesPreferred(it, targetSlot) }
        }
        if (
            vpnConfig == null &&
            !accessDenied &&
            hasVpnAccess() &&
            repo.hasMainVpnServerHashes() &&
            targetSlot == repo.getPreferredServer()
        ) {
        ...
        if (vpnConfig != null) {
            ...
            applySubscriptionDeniedFromApi()
            apiError = apiError ?: subscriptionRequiredMessage()
        }

        return ConnectFetchResult(vpnConfig, apiError, accessDenied)
    }

    private fun warmConnectFetch(slot: String = repo.getPreferredServer()): ConnectFetchResult? {
        val key = SilentRepository.normalizePreferredServer(slot)
        val p = prefetchedConnectBySlot[key] ?: return null
        if (p.accessDenied) return null
        val cfg = p.vpnConfig ?: return null
        if (!isConfigConnectable(cfg) || !cachedConfigMatchesPreferred(cfg, key)) return null
        return p
    }

    private fun rememberPrefetch(result: ConnectFetchResult, slotOverride: String? = null) {
        val slot = SilentRepository.normalizePreferredServer(
            slotOverride
                ?: SilentRepository.slotFromSelectedServer(result.vpnConfig?.selected_server)
                ?: repo.getPreferredServer(),
        )
        if (result.accessDenied || result.vpnConfig == null) {
            prefetchedConnectBySlot.remove(slot)
            return
        }
        prefetchedConnectBySlot[slot] = result
        result.vpnConfig.client_sync?.let { bundle ->
            // WG-ключи уже в cacheVpnConfig. Профиль UI — из живого /me, не из client_sync без даты.
            repo.applyClientSync(bundle.copy(profile = null))
        }
    }

    private fun applySubscriptionDeniedFromApi() {
        restoreCachedProfileToUi()
        markLocalSubscriptionInactive()
        // WG-кеш не трогаем: тумблер поднимает main VPN из кеша.
        prefetchedConnectBySlot.clear()
    }

    private fun rememberWarmConnectFromCache() {
        val cfg = loadCachedVpnConfig()?.takeIf {
            isConfigConnectable(it) && cachedConfigMatchesPreferred(it)
        } ?: loadSlotCachedVpnConfig(repo.getPreferredServer()) ?: return
        rememberPrefetch(ConnectFetchResult(cfg, null, false))
    }
    ...
    /**
     * Splash ждёт: bootstrap на анимации + /me и WG-кеш всех слотов.
     * Тумблер сюда не ходит.
     */
    suspend fun prefetchConnectConfigAtLaunch(context: Context) {
        connectPrefetchMutex.withLock {
            if (!repo.isLoggedIn()) return
            if (repo.isOlcrtcBypass()) return
            setLaunchBootstrapProgress(0.08f)
            val onMobile = repo.isOnMobileData()
            if (userOwnsMainVpn() && connectableCachedConfig() != null) {
                rememberWarmConnectFromCache()
                val fpOwned = runCatching { repo.getDeviceFingerprint() }.getOrNull()
                if (fpOwned != null) prefetchAllServerSlotsAtLaunch(fpOwned)
                setLaunchBootstrapProgress(1f)
                return
            }
            if (android.net.VpnService.prepare(context) != null) {
                DebugLog.i("MainViewModel", "launch prefetch skip config: no VPN permission")
                if (connectableCachedConfig() != null) rememberWarmConnectFromCache()
                setLaunchBootstrapProgress(1f)
                return
            }
            ...
            DebugLog.i(
                "MainViewModel",
                "launch prefetch /vpn/config slot=${repo.getPreferredServer()} mobile=$onMobile",
            )
            setLaunchBootstrapProgress(0.4f)
            ...
            if (fetch.vpnConfig == null) {
                rememberWarmConnectFromCache()
            }
            prefetchAllServerSlotsAtLaunch(fp)
            if (userOwnsMainVpn()) {
                DebugLog.i("MainViewModel", "launch prefetch finished after user connect — keep GETCONF")
            }
            setLaunchBootstrapProgress(1f)
        }
    }

    private fun loadSlotCachedVpnConfig(slot: String): VpnConfig? {
        val json = repo.getCachedVpnConfigForSlot(slot) ?: return null
        val parsed = runCatching { Gson().fromJson(json, VpnConfig::class.java) }.getOrNull() ?: return null
        if (repo.isLoggedIn() && parsed.device_id.startsWith("boot:")) return null
        if (parsed.device_id != repo.getSessionDeviceId()) return null
        return parsed.takeIf { isConfigConnectable(it) && cachedConfigMatchesPreferred(it, slot) }
    }
    ...
    private fun connectableCachedConfig(): VpnConfig? =
        warmConnectFetch()?.vpnConfig
            ...
            ?: loadSlotCachedVpnConfig(repo.getPreferredServer())
    ...
    private suspend fun awaitStartPrefetchForToggleCache() {
        ...
        if (job != null && job.isActive) {
            // Если кеша под текущий слот нет (часто сразу после «Применить сервера»),
            // короткое ожидание создаёт гонку: prefetch позже гасит bootstrap и
            // пользователь видит краткий OFF/ON системной иконки VPN.
            val hasReadyCache = connectableCachedConfig() != null
            val waitMs = if (hasReadyCache) 2_500L else 12_000L
            DebugLog.i(
                "MainViewModel",
                "toggle: wait launch prefetch finish (${waitMs}ms, cache=$hasReadyCache)",
            )
    ...
    private suspend fun refreshSubscriptionAtLaunch() {
        repo.clearTunnelApiBase()
        repo.useApiBase(repo.getPublicServerUrl())
        repo.invalidateApiClient()
        val live = repo.fetchProfileLive().getOrNull()
        if (live == null) {
            DebugLog.w("MainViewModel", "launch /me unreachable — keep cache, bootstrap will confirm")
            return
        }
        applyServerProfile(live, force = true)
        DebugLog.i(
            "MainViewModel",
            "launch /me active=${live.subscription.is_active} days=${live.subscription.days_left}",
        )
    }
    ...
    fun connect(context: Context, olcrtcReassignAttempt: Int = 0) {
        ...
        connectJob = viewModelScope.launch {
            ...
            bootstrapVpnMode = false
                ...
                // Конфиг с splash; тумблер не делает второй bootstrap.
                ...
                // Тумблер: сначала локальный кеш выбранного слота, при cache-miss — один fetch /config.
                // Без ephemeral/overlay, чтобы не дергать лишние перезапуски.
                ...
                if (readyConfig == null) {
                    ...
                    _subscriptionProbeActive.value = false
                    pendingGetconfProfile = null
                    _vpnError.value = if (!hasVpnAccess()) {
                        subscriptionRequiredMessage()
                    } else {
                        "Нет VPN-конфига. Закройте приложение и откройте снова."
                    }
                    _vpnState.value = VpnState.DISCONNECTED
                    return@launch
                }
                ...
                launchVpnService(context, toConnect, requireGetconf = false)
                pendingConnectAfterSubscriptionRefresh = !hasVpnAccess()
                waitForTunnelReady(
                    context,
                    toConnect.stream_count,
                    relaunchConfig = toConnect,
                    requireGetconfAccess = false,
    ...
    private fun cachedConfigMatchesPreferred(config: VpnConfig, wantSlot: String = repo.getPreferredServer()): Boolean {
        val want = SilentRepository.normalizePreferredServer(wantSlot)
    ...
    private fun launchVpnService(
        ...
        val intent = Intent(context, SilentVpnService::class.java).apply {
            ...
            putExtra(SilentVpnService.EXTRA_IS_BOOTSTRAP, isBootstrap)
            putExtra(SilentVpnService.EXTRA_REQUIRE_GETCONF, !isBootstrap && requireGetconf)
        }
        ContextCompat.startForegroundService(context, intent)
    }
    ...
    private suspend fun applyRefreshVpnConfigDirect(fp: String) {
        ...
        var cfg = runCatching {
            ...
            when {
                ...
                res.code() == 402 || res.code() == 403 -> {
                    if (userOwnsMainVpn()) return
                    lastVpnConfigDenied = true
                    applySubscriptionDeniedFromApi()
                    return
                }
    ...
    private suspend fun waitForTunnelReady(
        ...
        requireGetconfAccess: Boolean = false,
        ...
        for (attempt in 0 until 3) {
            if (attempt > 0) {
                ...
                _vpnState.value = VpnState.CONNECTING
                launchVpnService(context, relaunchConfig, requireGetconf = requireGetconfAccess)
            }

            val waitTicks = if (repo.isLegacyCaptchaStrategy()) 1200 else 450
            var ready = isTunnelReadyForConnect(requireGetconfAccess)
            var escalateEarly = false
            for (tick in 0 until waitTicks) {
                if (ready) break
                delay(100)
                if (_vpnState.value != VpnState.CONNECTING) return
                val err = WdttTunnelManager.lastError.value.orEmpty()
                if (isSubscriptionAccessError(err)) {
                    stopVpnLocally(context)
                    applySubscriptionDeniedFromApi()
                    awaitingGetconfAccess = false
                    pendingGetconfProfile = null
                ...
                if (isTunnelReadyForConnect(requireGetconfAccess)) {
                    ready = true
                    break
                }
            ...
            if (_vpnState.value != VpnState.CONNECTING) return
            if (ready || isTunnelReadyForConnect(requireGetconfAccess)) {
                if (requireGetconfAccess) {
                    awaitingGetconfAccess = false
                    markLocalSubscriptionActiveFromGetconf()
                }
                _vpnState.value = VpnState.CONNECTED
                onVpnTunnelReady()
                return
            ...
            if (!allowEscalation || relaunchConfig == null) break
        ...
        val subDenied = isSubscriptionAccessError(WdttTunnelManager.lastError.value.orEmpty())
        stopVpnLocally(context)
        withContext(Dispatchers.IO) {
            VpnConnectHelper.ensureCleanSlate(context, force = true)
        }
        if (requireGetconfAccess && subDenied) {
            applySubscriptionDeniedFromApi()
            awaitingGetconfAccess = false
            pendingGetconfProfile = null
            _subscriptionProbeActive.value = false
    ...
    private fun isTunnelReadyForConnect(requireGetconfAccess: Boolean): Boolean {
        if (!WdttTunnelManager.running.value) return false
        return if (requireGetconfAccess) {
            WdttTunnelManager.isGetconfWgUp()
        } else {
            WdttTunnelManager.tunnelReady.value
        }
    }
    ...
    private fun paymentInitUserMessage(e: Throwable?): String {
        ...
        if (raw.isBlank() || repo.isPublicConnectFailure(raw) || raw.contains("10.66.66") ||
            raw.contains("tunnel api", ignoreCase = true)
        ) {
            return "Не удалось открыть оплату. Повторите."
        }
        return raw
    }
    ...
    fun startPaymentPoll(label: String) {
        ...
        paymentPollJob = viewModelScope.launch {
            ...
            while (true) {
                ...
                if (hold && !repo.isMainVpnTunnelUp() && !isPaymentBootstrapTunnelUp()) {
                    runCatching { ensurePaymentBootstrapHeld(appContext) }
                }
                if (paid) {
                    markPaymentConfirmedUi()
    ...
    private fun userOwnsMainVpn(): Boolean {
        if (bootstrapVpnMode || silentBootstrapSync || WdttTunnelManager.isBootstrapMode()) {
            return false
        }
        if (repo.isMainVpnTunnelUp()) return true
        if (_vpnState.value == VpnState.CONNECTED) return true
        return _vpnState.value == VpnState.CONNECTING && SilentVpnService.isRunning
    }

    /** Тумблер в CONNECTING ещё ждёт кеш — это не захват main VPN. */
    private fun ephemeralShouldYieldToUser(): Boolean {
    ...
    private fun hasVpnAccess(): Boolean {
        if (repo.isVpnAccessDenied()) {
            val p = _profile.value
            return p?.is_admin == true
        }
        return hasVpnAccessForProfile(_profile.value)
    }

    private fun shouldRequireGetconfAccess(): Boolean =
        !hasVpnAccess() || lastVpnConfigDenied || pendingConnectAfterSubscriptionRefresh ||
            repo.isVpnAccessDenied()

    /**
     * Тумблер: GETCONF/DTLS — второй канал сверки подписки, без HTTP и без overlay.
     * Выдали WG — доступ есть, тумблер уже CONNECTED. DENIED обрабатывает lastError.
     */
    private fun markLocalSubscriptionActiveFromGetconf() {
        if (getconfConfirmStarted) return
        getconfConfirmStarted = true
        lastVpnConfigDenied = false
        pendingConnectAfterSubscriptionRefresh = false
        getconfGrantedThisVpnSession = true
        repo.setVpnAccessDenied(false)
        _vpnError.value = null
        _subscriptionProbeActive.value = false
        val pending = pendingGetconfProfile
        pendingGetconfProfile = null
        val paid = when {
            pending != null && profileHasPaidUi(pending) -> pending
            _profile.value?.let { profileHasPaidUi(it) } == true -> _profile.value
            else -> null
        }
        paid?.let {
            applyServerProfile(it, force = true)
            repo.saveCachedProfile(it)
        }
        DebugLog.i("MainViewModel", "GETCONF granted — keep main VPN, no HTTP recheck")
    }
    ...
    /** Есть чем нарисовать «Оплачено» / пробный / бессрочно — без фейка «Подписка активна». */
    private fun profileHasPaidUi(p: UserProfile): Boolean {
        if (p.is_admin) return true
        val s = p.subscription
        if (!s.is_active) return false
        if (s.plan_type == "unlimited" || s.plan_type == "trial" || s.plan_type == "test") return true
        return s.days_left > 0 || !s.expires_at.isNullOrBlank()
    }

    /** Main VPN уже поднят: сервер режет канал. Не гасим сами из stale-кеша «нет». */
    private fun shouldIgnoreInactiveAfterGetconf(profile: UserProfile): Boolean {
        if (hasVpnAccessForProfile(profile)) return false
        if (bootstrapVpnMode || silentBootstrapSync) return false
        return repo.isMainVpnTunnelUp()
    }

    private fun shouldKeepGetconfSessionDespiteInactiveProfile(profile: UserProfile): Boolean =
        shouldIgnoreInactiveAfterGetconf(profile)
    ...
    private fun applyServerProfile(profile: UserProfile, force: Boolean = false) {
        ...
        if (vpnFullyUp && !hasAccess) {
            if (awaitingGetconfAccess) {
                DebugLog.i("MainViewModel", "GETCONF still confirming — ignore inactive cache")
                return
            }
            MobileSyncLog.w(
                "profile",
    ...
    private fun isSubscriptionAccessError(message: String): Boolean {
        ...
        return lower.contains("подписка недействительна") ||
            ...
            lower.contains("срок действия пароля") ||
            (lower.contains("истёк") && lower.contains("парол")) ||
    ...
    private fun markLocalSubscriptionInactive() {
        ...
        repo.saveCachedProfile(next)
        _profile.value = next
        DebugLog.i("MainViewModel", "local subscription marked inactive")
    }
