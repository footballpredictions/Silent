...
Статус на 2026-09-08: `admin_only` снят. Слот «Сервер 4 для ИИ» в `GET /api/vpn/servers`
только у клиентов **1.0.165+** (`X-App-Version`). 1.0.164 и ниже слот не видят.
WARP/TPROXY выключены (fail-open, прямой HOSTKEY). WDTT-spill на `ai_exit` не льёт.
...
3. Снять `admin_only` кнопкой в Улье («Открыть всем») **только после** деплоя
   API с гейтом `AI_EXIT_MIN_CLIENT = 1.0.165`. Иначе 1.0.164 увидит слот.
