...
func TestWorkerStartDelayStaggersHalfSecond(t *testing.T) {
	if workerStartDelay(0) != 0 {
		t.Fatal("first worker starts immediately")
	}
	if workerStartDelay(8) != 4*time.Second {
		t.Fatalf("last worker of group want 4s stagger, got %v", workerStartDelay(8))
	}
}
