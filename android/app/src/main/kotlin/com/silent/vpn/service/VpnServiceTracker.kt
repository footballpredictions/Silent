...
/** Синхронизация VPN-сессии с системой и плиткой QS (persist + getRunningServices). */
object VpnServiceTracker {
    ...
    fun reconcileStaleSession(context: Context): Boolean {
        ...
        try {
            ...
            if (!staleMemory && stalePref) {
                SessionTrace.warn(
                    "VpnServiceTracker.reconcileStaleSession",
                    "pref active service dead — try restart",
                )
                if (VpnTileConnect.restartCachedSessionDirect(appCtx)) {
                    SessionTrace.exit("VpnServiceTracker.reconcileStaleSession", "restarted from pref")
                    return true
                }
            }
