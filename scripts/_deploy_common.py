"""SSH/VPS для deploy Android APK (OTA)."""
...
def load_env() -> None:
    for path in (
        ANDROID_ROOT / ".env.deploy",
        ANDROID_ROOT.parent / ".env.deploy",
...
def connect(timeout: int = 30):
    ...
    if not password:
        raise SystemExit("Задайте DEPLOY_PASS в Silent-Project/.env.deploy")
