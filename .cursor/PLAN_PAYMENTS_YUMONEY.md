...
**Единый флоу для всех клиентов (обязательно):** PC и Android (и в будущем iOS) реализуют **одинаковую** логику — init → открыть `url` в системном/внешнем браузере → poll `/status/{label}` → одинаковые состояния (waiting/success/failed/timeout) из одинаковых theme-полей. Никакой платформенной самодеятельности в самом флоу, разница только в UI-компонентах.

**Открывать напрямую в браузер, не проксировать через наш бекенд:** `url` из `/payments/init` — это прямая ссылка на `yoomoney.ru/quickpay/confirm.xml`; клиент передаёт её системному браузеру как есть (PC: `electronAPI.openExternal`, Android: `Intent.ACTION_VIEW`/Custom Tabs). Наш бекенд не встраивает форму оплаты в собственную страницу и не отображает её во WebView/iframe — только `successURL` (после оплаты) — это страница нашего бекенда (§6.5), потому что именно туда YuMoney обязан сделать редирект, и она обязана быть публичным URL. Сама оплата от начала до нажатия «Оплатить» — целиком на стороне YuMoney в браузере.
...
1. URL: `https://89-125-188-100.nip.io/api/payments/yumoney/notify`
