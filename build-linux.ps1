...
if ($WithDebugLog) {
  Write-Host 'DEBUG UI: Log/Hashes ENABLED (temporary build)' -ForegroundColor Yellow
  Set-Content -Path 'src\main\buildFlags.js' -Value 'module.exports = { DEBUG_BUILD: true };' -Encoding ascii
  $env:DEBUG_BUILD = "1"
} else {
...
# Always restore release flag so the next Windows build does not ship with Log UI.
Set-Content -Path 'src\main\buildFlags.js' -Value 'module.exports = { DEBUG_BUILD: false };' -Encoding ascii
Remove-Item Env:DEBUG_BUILD -ErrorAction SilentlyContinue

Write-Host '=== LINUX BUILD SUCCESS ==='
if ($WithDebugLog) {
  Write-Host 'NOTE: this .deb has DEBUG log UI - do not publish as production OTA' -ForegroundColor Yellow
}
