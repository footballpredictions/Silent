...
## Последние изменения (egress Улей + сота 3 2026-09-19)

После смены IP Улья NAT на `89.125.188.100` живой, хвоста `132.243` нет. Соты 1–2 «летали» не из‑за отдельного прокси, а из гигиены: TTL 64, IPv6 DROP, `tcp_mtu_probing=1`; сота 2 ещё TCPMSS clamp (game-exit). Улей и сота 3 этого не имели — через WG MTU 1280 часть HTTPS (4PDA и др.) рвалась/тормозила. На Улей и соту 3 залил тот же PMTU-тюнинг (`TCPMSS --clamp-mss-to-pmtu`, TTL 64, IPv6 DROP, probing), **wdtt не рестартил**. Соты 1–2 не трогал. Переподключить Сервер 1.
...
Деплой `deploy_stable.py` 18.09: tunnel health OK, wdtt active, health 0.044с, `queen wg kick` 0 / 20с. Push: `main` `416d4ce`, `pc` `b675b04`, `android` `630481c`, `ios` `fa5e59a`, `openwrt` `53f029b`. Версию не поднимал, OTA-сборки не пересобирал.
...
не видно, подтянулся ли конфиг. Splash без WG-кеша теперь сразу bootstrap и на Wi‑Fi.
Тумблер по-прежнему сначала соты. Версию не поднимал.

## Последние изменения (Android stale WG Сервер 1 после OTA 2026-09-18)

После 165→166 тумблер Сервера 1 крутился, соты вставали сразу: кеш WG оставался
со старым IP Улья `132.243.234.162`, logout сбрасывал сессию. Теперь: кеш слота
не берём, если IP ≠ bake; одноразово чистим slot-cache; таймаут коннекта тоже
снимает кеш. Версию не поднимал. **wdtt/api не трогал.**
...
Публичный API: соты `:9100` первыми, Улей `:443` последним — Android, PC,
iOS, OpenWrt. Bootstrap PC/Android: Endpoint живой соты, полный туннель
(не только `10.66.66.0/24`). Skip подтверждения почты — админка + все
клиенты. Письма на проде через соту после handshake SMTP.

## Последние изменения (деплой через соту 2026-09-17)

`:9100` не деплой. Если `:22` Улья с РФ мёртв и VPN на соте (`10.66.66.1` —
шлюз соты), SSH: ПК → Сота 1/2 `:22` → Улей `:22`. `DEPLOY_JUMP_PASS` или
тот же `DEPLOY_PASS`.

## Последние изменения (SMTP AUTH на соте 2026-09-17)

Письмо после деплоя не уходило: Улей POST `/v1/smtp-send` на соты → **502
`SMTPNotSupportedError`**. TCP/TLS к `smtp.mail.ru:465` с соты живые, но
handshake не читал баннер `220`, EHLO его съедал, AUTH не было. Починен
`handshake_smtp` (тест `test_open_smtp_465_reads_220_banner_before_ehlo`).
Сота 1/2 `silent-cell-agent` перезапущен, **wdtt не трогали**. Проба после
фикса: `SMTPRecipientsRefused` на example.com — логин mail.ru проходит.
Полный `deploy_stable` (API `email_smtp.py`) ещё не катили — для fallback
с Улья нужен «деплой».

## Последние изменения (почта через соту как bootstrap 2026-09-17)

Регистрация через соту проходит, письма с Улья нет: smtp.mail.ru с Сервера 1
недоступен. Отправка: Улей сразу POST `/v1/smtp-send` на публичный `:9100`
той же соты (Сота 1 первая), SMTP с Улья только если соты не ответили.
Ссылка в письме: кнопка = `http://сота:9100/api/auth/verify-email` (прокси
на Улей), nip.io:443 последней. На прод залито 2026-09-17: SSH-прыжок
ПК → Сота 1 `:22` → Улей (`DEPLOY_JUMP_PASS`). Health 200, DNAT на API,
wdtt active, `queen_wg_kick_20s=0`.

## Последние изменения (skip подтверждения почты 2026-09-17)

Админка **Доп. настройки** — тумблер «Отключить подтверждение почты» (дефолт выкл).
Когда включён: регистрация сразу `is_verified`, письмо не шлётся, вход без ссылки.
Старые клиенты 1.0.160/161 могут войти (логин больше не 403). Новые Android/PC/OpenWrt
после регистрации сразу логинятся. Поле темы `skip_email_confirmation` опционально.

## Последние изменения (Gmail письма с бэка 2026-09-17)

На Wi‑Fi тоже пусто — клиент ни при чём. Две дыры отправки:
1. From = `noreply@silent-vpn.ru` через `smtp.mail.ru` → Gmail/Yahoo дропают
   (SPF/DMARC). From берём домен SMTP-ящика.
2. Сначала ждали Улей 12с (часто ENETUNREACH), сота была запасной. Теперь
   живые соты `/v1/smtp-send` шлют сразу.

## Последние изменения (Gmail IPv6 на temp VPN 2026-09-17)

Gmail на временном интернете не открывался: WG только `0.0.0.0/0`, Gmail
уходит в IPv6 мимо туннеля на LTE (ТСПУ). `::/0` в конфиг WG нельзя.
На полном bootstrap включён `VpnService.setBlocking` — IPv6 underlay
режется, Gmail падает на IPv4 через соту.

## Последние изменения (почта на temp VPN 2026-09-17)

Регистрация шла, письма в почтовом клиенте на временном интернете — нет.
Bootstrap ставил `includeApplications` (короткий список почты). На vivo это
часто no-op: в туннеле только Silent VPN, почтовый клиент остаётся на LTE.

Теперь overlay — полный туннель, наружу только VK. Любой почтовый клиент
(в том числе стоковый) получает интернет соты.

## Последние изменения (register 404 через соту 2026-09-17)

Временный VPN на соте жил, регистрация писала «API endpoint not found».
Клиент на `10.66.66.1:8000` ставил `Host: nip.io` → HTTP :80 Улья делал
301 на HTTPS. OkHttp менял POST register на GET; catch-all Hive → 404.
Health GET после 301 оставался GET и проходил — таймер шёл.

Фикс: Host nip.io только на публичный IP; туннель без Host-rewrite.
Nginx :80 nip.io проксирует `/api/` и `/health`, 301 только страницы.

## Последние изменения (temp VPN через соту 2026-09-17)

Смена `:2083` — только HTTPS API, не UDP Сервера 1 (`:56000`). Регистрация
поднимала overlay на Улей — handshake флапает, почта в includeApplications
с узким AllowedIPs в blackhole.

Debug: bootstrap Endpoint = живая сота (87.58…), AllowedIPs `0.0.0.0/0` только
на соте. На Улей overlay по-прежнему узкий, чтобы не глотать серверы 2/3.

## Последние изменения (порт при 1/2 2026-09-17)

С РФ 443 то жив, то мёртв (1/2 check-host). Политика считала это флапом и не
публиковала запасной порт — окно 0/2 так и не набиралось.

Теперь при **любой** резке с РФ (`tcp_fail` или `tls_fail` > 0) открываем `:2083`
в теме. **443 не закрываем** — старые клиенты остаются на нём, новые берут соты
и затем `:2083`. Кандидатов зовём уже на 1/2. IP-blackhole и local 443 down — как были.

## Последние изменения (debug APK blackhole 2026-09-17)

Overlay bootstrap с `AllowedIPs=0.0.0.0/0` на WG Улья :56000: handshake нет → HTTP к сотам
и UDP серверов 2/3 в blackhole. Откат: снова `10.66.66.0/24` + IP Улья `/32`. Соты :9100
в публичном failover остаются первыми. Релизный клиент этот баг не содержит.

Сервер 1 на релизе: Endpoint всегда Улей UDP `:56000`. Карточка 1/2 TCP — флап 443,
UDP с РФ агент не пробует. Если ТСПУ режет и 56000, Сервер 1 с РФ не встанет без mesh.

## Последние изменения (автосмена порта API 2026-09-17)

Карточка «Запасной порт API: Держать 443» не врала: смена **намеренно не работала**.
`build_plan` зашивал `autoswitch=False`, `executed=False`, кандидатов не звали
пока `dead_windows+1 < confirm` (при confirm=3 и одном окне — «не проверяли»),
`HIVE_API_PORT_AUTOSWITCH` на проде был false, исполнителя не было.

Сделано:
- Кандидатов с РФ зовём с **первого** мёртвого окна.
- Автосмена в compose `true`; политика получает флаг; confirm порта = 2.
- Исполнитель `ai/hive_api_port_exec.py`: пишет `2083` в `static/.hive_api_alt_ports`,
  только если nginx локально слушает. 443/22/56000 не закрываем, wdtt не трогаем.
- Тема читает файл без recreate API. Соты `:9100` по-прежнему первые.
- Если `:2083` уже в теме — второй кандидат не открываем.

После деплоя: «Проверить сейчас». При streak≥2 и живом 2083 с РФ карточка
«опубликован :2083». Если РФ режет и 2083 — не публикуем (мёртвый URL хуже).

## Последние изменения (письма + Сервер 1 + overlay 2026-09-17)

Корень трёх жалоб один: при блоке 443 Улья клиент либо не брал соты, либо bootstrap резал интернет почте.

- Почта: bootstrap `includeApplications` (Gmail/Mail.ru) + AllowedIPs только `10.66.66.0/24`+IP Улья → IMAP/HTTPS почты в blackhole. Теперь 0.0.0.0/0 (TURN-дыры как были).
- Анимация/подписка: splash сразу поднимал ephemeral на Улей `:56000` и не бил в соты `:9100`. Сначала публичный failover (соты), Hive WG только если соты молчат.
- Старые клиенты: запасной HTTPS Улья `:2083` (nginx listen + compose publish), в теме `HIVE_API_ALT_PORTS=2083` после живых сот. 443/wdtt не закрываем.
- PC: тот же порядок баз — соты, потом Улей.

## Последние изменения (письма подтверждения 2026-09-17)

Регистрация на проде проходит (201), а письма часто не доходят. Логи API 48ч:
SMTP = `smtp.mail.ru:465` (логин ок), но пачками `Email send failed: [Errno 101] Network is unreachable` через ~2–4 мин после Register. `SMTP_SSL` был **без timeout** — SYN в никуда вешал BackgroundTasks, следующие письма ждали в очереди. За 48ч 4 неподтверждённых.

Фикс **на проде** 2026-09-17 (`deploy_stable.py`, SSH со 2-й попытки):
app 117 py, ai 18 py, cell-agent (`/v1/smtp-send`), admin-ui.
`health OK` (58 мс), `hive/cells 401`, `wdtt active`, `queen_wg_kick_20s=0`, туннель `10.66.66.1:8000/health` OK.
Соты подтянут агент автоапгрейдом; до этого Улей уже шлёт с IPv4+timeout 12с, релей на соту — после апгрейда.

## Последние изменения (кольцо серверов + хотфикс писем 2026-09-16)

Как уже устроено: **API** — клиент бьёт в Улей:443, при таймауте в соты `:9100`, сота проксирует на Улей `IP:8000`/`:80`. Сота 3 из этого списка исключена (`ai_exit`, `:9100` только Улью). **VPN** — не кольцо: `/config` отдаёт один Endpoint = публичный IP выбранного сервера; у каждой соты свой `wdtt0` и свой `10.66.66.1`. Подменить Endpoint на Соту 1 без туннеля Сота1→Сота3 = выход Соты 1, не ИИ.

Поэтому автосмену портов и TLS клиент→сота **не включали**: флап 443 + публикация мёртвых `:2083` уже сломали письма; TLS на `:9100` сломает клиентов 1.0.160/161 (HTTP). Исполнитель портов и `silent-mesh0` — отдельные задачи.

Сделано:
- Регресс темы: `HIVE_API_ALT_PORTS=""`; `compose_standby_api_urls` ставит соты **перед** alt-портами; в письме один хост = одна ссылка, IP Улья в резерв не берём.
- Сота→Улей: `queen_api_bases` = `:8000` → `:80` → HTTPS nip.io (IP первый).
- Dry-run релея: `ai/cell_relay_policy.py` + карточка в админке. Релей только если WDTT/WG с РФ точно мёртв; нет пробы → прямой ход. `executed=False`.

**Задеплоено 2026-09-17** (`deploy_stable.py`, SSH со 2-й попытки через `10.66.66.1`):
app 115 py, ai 18 py, cell-agent, admin-ui. `health OK` (~39 мс), `hive/cells 401`, `wdtt active`, `queen_wg_kick_20s=0`, туннель `10.66.66.1:8000/health` OK.
Тема с соты: `hive_standby_api_urls=http://87.58.213.193:9100,http://78.17.74.27:9100` — мёртвых `:2083/:2053` больше нет.

## Нет конфига / тумблер без overlay (2026-09-17)

Тумблер брал `/config` только с Улья `nip.io:443`. Проба «API жив» смотрела и соты `:9100` — они отвечают, overlay не стартовал, запрос всё равно уходил в заблокированный 443 → «нет конфига».
Android: `fetchVpnConfigViaPublicFailover` (Улей 4с → соты 8с), список серверов тоже; overlay если и Улей и соты молчат. `isHiveHttpsReachable` для пропуска overlay.

## Аудит Улья 2026-09-16: вторжения нет, вход цел, режут снаружи

`scripts/audit_hive_security.py` + `scripts/diag_hive_tls_443.py` (оба read-only,
ничего не рестартили). SSH поймался с 6-й попытки: TCP встаёт, баннер теряется —
блок **плавающий**, не постоянный.

Следов вторжения нет: `ufw inactive`, `-P INPUT ACCEPT`, **DROP/REJECT на 443/22
отсутствуют** (значит блок не наш); `authorized_keys` пустые с 25.05; входы только
`79.111.231.32` (админ-ПК) и `87.58.213.193` (Сота 1); неудачные — обычные боты
(`shiloh`/`admin` с 2.57.121.112, 193.46.255.86, 92.205.229.83); `ld.so.preload`
пусто; в `/tmp` только наши `restore_api_code.sh` / `fix_tp.sh`; контейнеры штатные
(api, nginx, db, redis, silent-nsenter); cron — certbot/acme; `wdtt active`, 196 peer'ов.

Вход на Улье **цел**: с правильным Host `https443(host):200`, хайрпин на свой
публичный IP тоже `200`; `openssl s_client` отдаёт `CN=132-243-234-162.nip.io`,
TLSv1.3, сертификат до 28.11.2026; `api8000:200`. Осторожно с трактовкой:
`curl https://127.0.0.1` даёт **000**, а `http://127.0.0.1` — **403**, и это
штатно: на 443 `listen default_server` → `return 444` для чужого Host, на 80
allow-list сот. Проверять только с `--resolve` на домен.

Итог: недоступность 443/22 из РФ — внешний фильтр (ТСПУ/хостер), плавающий по
соединениям. Ни наш фаервол, ни поломка сервиса, ни взлом.

Классификатор: добавлен `tls_entrance_broken` — если локально TCP 443 открыт,
а TLS+HTTPS мертвы, это `service_down` («вход по TLS сломан у нас»), и port_block
для 443 не выводится. До правки такой расклад давал «Блокировок не обнаружено».
Тесты `test_local_tls_dead_is_our_breakage_not_port_block`,
`test_live_tls_locally_still_allows_port_block`; `test_availability_unit.py` 44 ok.

Dry-run запасного порта: `ai/availability_port_plan.py` + `port_plan` в отчёте и
карточка в `HiveAvailabilityPanel.tsx`. Агент считает решение и показывает его,
**портов не открывает** (счётчик мёртвых окон — `availability_agent_port_state`).
Кандидаты пробуются с РФ только когда блок близок к подтверждению.
Тест `test_availability_port_plan_unit.py` (7 ok).

Грабли в своём же аудите: `ps aux` печатал `-password`/`-internal-secret` wdtt —
добавлена маскировка, лог с секретами удалён. Аудит-вывод в репозиторий не кладём.

SSH-деплой при плавающем блоке: `_deploy_common.connect()` не ловил
`paramiko.SSHException` (потеря баннера) и не повторял попытки — деплой падал
на первой неудаче. Теперь повторы по обоим хостам, `banner_timeout`/`auth_timeout`,
и env `DEPLOY_SSH_ATTEMPTS` / `DEPLOY_SSH_PAUSE_SEC` (по умолчанию 4 × 6 с).
Окно 16.09 открылось только через ~10 мин ожидания.

**Задеплоено 2026-09-16** (`deploy_stable.py`, `admin-ui` пересобран):
app 114 py, ai 17 py, mounts 2/2, `health OK`, `hive/cells 401`, `wdtt active`,
`queen_wg_kick_20s=0`, DNAT туннеля проверен (`OK tunnel 10.66.66.1:8000/health`).
В прод ушли: письма с резервными ссылками, `tls_entrance_broken` в классификаторе,
dry-run `port_plan` + карточка в админке.

## Последние изменения (письма мимо 443 + аудит Улья 2026-09-16)

Регистрация работала, а подтверждение — нет: письмо всегда несёт одну ссылку на
`FRONTEND_URL` (nip.io:443). При резке 443 из РФ подтвердить/сбросить пароль
нечем, хотя `/api/auth/*` на сотах `:9100` проксируется на живой Улей.
Фикс backend-only (клиенты не трогаем): `app/services/email_links.py` +
`_email_fallback_bases()` в `auth.py` → в письме основная ссылка на Улей и до
2 резервных на соты. Туннель `10.66.66.1` и localhost в письмо не попадают.
Тест `scripts/test_email_links_unit.py` (6 ok).

Смена портов: «мозг» уже был — `ai/hive_api_port_policy.py` (1/2 нод = HOLD,
443/22/80/8000/56000/56001 в `KEEP_FOREVER`, кандидаты 2083/2053/2087/2096/8443),
8 тестов. Нет исполнителя: `decide_api_port_action` **никто не вызывает**,
`HIVE_API_PORT_AUTOSWITCH` — мёртвый флаг, `scripts/open_hive_alt_https.py`
из комментария не существует. Клиентам менять код **не нужно**: они уже берут
любые базы из темы `hive_standby_api_urls` (Android prefs, PC `set-standby-api-bases`),
а `hive_standby.hive_alt_api_urls()` умеет публиковать `https://host:порт`.

Аудит: `scripts/audit_hive_security.py` — read-only (ufw/iptables про 443/22,
счётчики пакетов, входы SSH, ключи, cron/timers, ld.so.preload, docker, mtime
кода, топ IP в nginx). Ничего не рестартит. Нужен SSH: публичный `:22` в таймаут,
`10.66.66.1:22` — это сота, поэтому VPN должен стоять на слоте **Улья**.

Дыры, найденные по пути (не чинили): `verification_token` без TTL (в письме
«24 часа», в коде срок не проверяется); хоп клиент→сота `:9100` и сота→Улей
`:8000` — **plaintext HTTP**, то есть сейчас пароли/JWT идут по интернету
открыто, пока вход живёт через соты.

## Последние изменения (Улей :443 из РФ 2026-09-16)

«VPN лёг» — не падение Улья. С этой сети: `10.66.66.1:8000/health` **200**,
Сота 1/2 `:9100/health` **200**, `sync-state` **401** (живой Улей, не снимок).
Улей `:80` TCP открыт (~70 мс), `:443`/`:22`/`:8443` timeout, HTTPS nip.io timeout.
Серверы 2 и 3 «заработали», потому что клиенты ушли на соты `:9100`.
Карточка «Блокировка по порту 443» верная по факту, решения DNAT 8443→443 —
нет: пакеты на 443 не доходят, клиенты без обновления всё равно бьют в :443.
Классификатор: при мёртвом API :443 не советовать DNAT на тот же IP; вход =
соты `:9100` + туннель. Тест `test_hive_443_port_block_does_not_recommend_dnat_on_same_ip`,
42 ok. Деплой классификатора — когда SSH Улья будет (публичный :22 режется;
`10.66.66.1:22` это сота, пароль Улья не подходит). wdtt не трогали.
...
Poll через publicHiveApi (не 10.66.66.1), JWT даже если слот устройства
не сразу встал. Задеплоено, приёмка ок.

## Последние изменения (QR ТВ: publicHiveApi + poll 403 2026-09-14)

Предыдущие тесты не били в реальный `getApi()`: при bootstrap VPN
`getServerUrl()` = `10.66.66.1`, а 403 (лимит устройств после approve)
крутился как «нет связи». Теперь QR ходит своим `publicHiveApi()` на
nip.io; 4xx показывается; poll всегда отдаёт JWT даже если слот устройства
не встал. Нужны новый APK на ТВ **и** деплой backend.

## Последние изменения (QR ТВ: public poll + не отменять свою job 2026-09-14)

Телефон confirmed, ТВ молчит даже после overlay-фикса:
1) poll/вход шли на 10.66.66.1, а приложение в ЧС — телефон ходит на публичный URL;
2) после токенов `qrPollJob.cancel()` убивал ту же корутину до `goToMain`.
Теперь poll как у телефона: public hive, overlay запасной; статус «Подтверждение получено».
Debug APK на ТВ.

## Последние изменения (QR ТВ: симуляция nested overlay 2026-09-14)

Телефон «уже зареган», ТВ молчит: poll стартовал *внутри* overlay `qrStart`.
`withApiOverlay` при уже включённом overlay не брал аренду, родитель гасил
маршрут — poll шёл мимо туннеля. Симуляция `QrTvLoginSimTest` это ловит.
Фикс: lease (refcount) + poll *после* qrStart. Debug APK на ТВ.

## Последние изменения (QR poll через bootstrap overlay 2026-09-14)

Телефон подтверждает, ТВ молчит: приложение вне WG, poll шёл на 10.66.66.1
без overlay — пакеты не в туннель. Теперь poll держит API overlay на всём
ожидании (те же 3 мин временного VPN). Debug APK на ТВ.

## Последние изменения (QR ТВ не входил после confirm 2026-09-14)

Телефон «подтверждён», ТВ молчит: poll удалял сессию с первого раза
(повтор = expired), плюс ТВ пересоздавал QR при мерцании bootstrap.
Poll теперь повторяемый до TTL; ТВ не меняет код, пока он жив.
Деплой stable, wdtt не трогали. Debug APK: `android/SilentVPN-debug.apk`.

## Последние изменения (QR только Smart TV + confirm 2026-09-14)

QR на входе — только Android TV. PC / OpenWrt / iOS / телефон: вкладка убрана.
Встроенный сканер удалён: любой камерный сканер открывает `silentvpn://qr`
(intent-filter уже был). В приложении кнопка «Подтвердить вход на ТВ».
Если VPN уже включён — сессию не рвём; если нет — короткий bootstrap, потом
гаснет. Debug: `android/SilentVPN-debug.apk`.

## Последние изменения (QR: скан молчал — битый encoder 2026-09-12)

Самодельный QR на PC/OpenWrt камера не читала (jsQR = null) — «ничего
не происходит» и с VPN, и без. Encoder заменён на `qrcode` (MIT).
Телефон (уже вошедший) шлёт `POST /api/auth/qr/approve` со своей сессией;
ТВ/PC/роутер забирает токены через `GET /qr/poll`. Деплой backend не нужен
(эндпоинты уже на Улье). Android: toast на всех путях + канонический URI.
Debug: `android/SilentVPN-debug.apk`, PC `build-debug-14557`.

## Последние изменения (QR сканер: иконка + полный экран 2026-09-12)

Кнопка сканера — иконка 28dp (не слово), заголовок не уезжает.
Оверлей на весь экран, TextureView (не SurfaceView — дыра/прозрачная полоса
в диалоге), рамка и бегущая полоска. Сканер только на Android-телефоне.
...
попадали и пригласившие с бонусом. Теперь только invitee со status=rewarded
и живым +1 мес (**5**). Оплаченные предпочитают живую покупку над длинным
бонусом. Фильтр «Рефералы (+мес)» — та же семантика.

## Последние изменения (админ: год не плюсует хвост 2026-09-09)

Выдача чипами в «Подписки» плюсовала хвост старого плана/рефа → год
от 09.09 давал **09.03.2028** (547 дн.) у `my@silent27-99.ru`. Оплата уже
не стекала с рефом; админ — стекал. Фикс: `admin_grant_expires_at` всегда
от now; revoke снимает все `status=active`. Бэкфилл: тот yearly → до 09.09.2027.
Тест в `test_plan_calendar_months_unit.py`. Деплой `deploy_stable.py`.
Аудит: `python scripts/audit_subscriptions_report.py`.

## Последние изменения (PTR/geofeed/ASN отложено 2026-09-09)

Пока Gemini/веб на Сервере 4 ок — PTR, geofeed и вторая нода в другом ASN
не делаем. Имеет смысл только если полезет капча/бан IP или понадобится
запасной провайдер. TASKS закрыт как отложено.

## Последние изменения (Профиль для ИИ из админки 2026-09-09)

Кнопка в Улье переименована: **«Профиль для ИИ»** / «Снять…» (бейдж «Для ИИ»).
Включение = флаг `ai_exit` + фон hygiene+dns на соте (без WARP/прокси).
Снятие = rollback all. Нужен сохранённый SSH. «Проверить IP» — только диагностика.
Клиентский слот по-прежнему «Сервер N для ИИ». CLI `deploy_ai_cell.py` остаётся для proxy/отладки.

## Последние изменения (Harden сот 1–2 2026-09-09)

На Соте 1 (`87.58.213.193`) и Соте 2 (`78.17.74.27`) применена базовая
гигиена: `ufw` active (22, 56000/01 udp, 9100, `10.66.0.0/16`),
`DEFAULT_FORWARD_POLICY=ACCEPT`, TTL→64, IPv6 FORWARD DROP, юнит
`silent-cell-hardening` enabled+started. Проверка: agent 9100=200, wdtt
active, tunnel proxy 200. `wdtt` не рестартовали. Сота 3 уже была под
AI-hygiene. PTR/geofeed/ASN — отложено (пока всё работает).

## Последние изменения (tunnel-api-proxy после ребута 2026-09-09)

После ребута Соты 1 с `lo` пропадал `10.66.66.1/32` → socat
`Cannot assign requested address`, NRestarts 300+. Починено на всех
рабочих сотах (1/2/сота3): unit с `ExecStartPre` (вернуть адрес на lo),
`network-online.target`, restart proxy **без** рестарта `wdtt`.
Проверка: proxy active, NRestarts=0, `lo` имеет 10.66.66.1/32, wdtt active;
tunnel `/health` Сота1/2 → 200, сота3 → 403 (HTTP дошёл, bind ок).
Шаблон в `hive_provision_service.py` для новых сот. Hardening 1–2 — отдельно.

## Последние изменения (Сота 1: не клиент повесил хост 2026-09-09)

Гипотеза «мой телефон повесил сервер» — по журналу **не сходится**.
Prev boot: 0 OOM/panic/soft-lockup; последние строки 21:27 — wdtt жив,
WRAP с десятков IP, Active 148; journal unclean → hard freeze/force reboot
(21:49), не штатный shutdown. Отдельно на соте: `silent-tunnel-api-proxy`
шторм рестартов (bind 10.66.66.1:8000 fail, NRestarts 200+) — **починено**
(см. выше). Hardening на соте 1 по-прежнему не установлен.

## Последние изменения (Сота 1 после сброса VPS 2026-09-09)

Корень — недоступность/флап VPS `87.58.213.193`, не баг учёта онлайна.
После сброса хоста: с Улья `9100` → 200 (~80 мс), ping 0% loss.
«Онлайн 1» при кажущемся дауне мог быть реальным keepalive, пока сота
кратко/частично поднималась; soft-online только усиливал путаницу в UI.

## Последние изменения (призрак online на Соте 1 2026-09-09)

«Онлайн 1» при мёртвой Соте 1 — не взлом. Устройство Vivo V2520A
(`silent27@bk.ru`, pin server2). WG peer ни на Улье, ни на Соте 2 нет, но
Queen `wdtt` шлёт `/internal/online` keepalive (~60с) — сессия в `activeDevices`
после обрыва/499 disconnect. Soft-online по пину клеится к Соте 1.
Сброс `is_connected` в БД без latch через ~70с возвращался. Нужен force-stop
приложения на Vivo (или ребут телефона), иначе keepalive поднимет флаг снова.

## Последние изменения (Сота 1 флапает после ребута 2026-09-09)

После ребута VPS `87.58.213.193` кратко поднималась (ping/9100/клиент Xiaomi),
потом снова **полностью** недоступна с Улья: ping loss, health 000, SSH timeout —
30+ с подряд. Это сеть/хост провайдера, не cell-agent.
Админский «онлайн» на Соте 1 — soft `is_connected` (таймаут 10 мин) + heartbeat
через API Улья; при мёртвом IP соты телефоны всё равно «светятся».
Клиентам: временно Сервер 1 (Улей) или Сота 2, не Сота 1.

## Последние изменения (Сота 1 offline + порядок в DPI 2026-09-08)

Сота 1 `87.58.213.193` целиком недоступна: с Улья и с ПК нет ping/SSH:22/TCP:9100
(timeout). Это не cell-agent — хост/сеть провайдера. Удалённо не поднять.
- В DPI-отчёте порядок целей: Улей → Сота 1 → Сота 2 → сота3 (раньше сота3
  была первой из‑за порядка UUID в БД).
- Вердикт `service_down` при мёртвом ping пишет «хост недоступен» + «перезагрузить VPS».
- Тест: `test_availability_unit.py` 40 ok.
...
- Верный сценарий: 27.08 quarterly → **27.11**, реф сверху → **27.12**.
- Бэкфилл: 1 пользователь (478 ₽ quarterly) 25.12 → оплата **27.11** + реф **27.12**. Скрипт: `fix_referral_paid_stack_expires.py`.
- Реф «до 26» при месяце до 27: после календарного бэкфилла оплаты реф оставался `+30` от старого конца. Добито: **5** активных рефов → то же число (+1…+2 дня). Скрипт: `fix_referral_on_paid_calendar.py`.

## Последние изменения (подписки: календарные месяцы 2026-09-07)
...
- Причина: `NetworkCallback` слушает **все** не-VPN сети. Сота, которую Android сам поднимает и гасит при живом Wi-Fi, ставила `lastBlackoutAtMs` в `onLosing` → `wifi_gap_restored`; она же роняла общий флаг VALIDATED → `validated_after_gap`. Обе причины идут в полный `restartTransport`, где защита по живым воркерам не работает.
- Фикс: `NetworkRecoveryPolicy.isOurUnderlyingNetwork(eventFp, currentFp, lastFp)` — события чужой сети игнорируются в `onLosing` и в учёте VALIDATED. Пороги (60 с окно дыры, 3.5 с VALIDATED) **не** трогали: после фикса метка ставится только на реальную дыру своей сети.
- Проверка по логу: при неподвижном телефоне не должно быть `gap restore wifi_gap_restored` и `network recovery: validated_after_gap`. Тест: `NetworkRecoveryPolicyTest`.
...
- Миграция `sota3_admin_only_v1` включает флаг для «Сота 3». В Улье кнопка «Только админ» / «Открыть всем».

## Последние изменения (Подписки = дашборд: kinds 2026-09-06)

- Расхождение «Выданные» 45 (дашборд) vs 36 (фильтр): дашборд клал `referral_bonus` в granted, фильтр брал только PAID_OR_GRANTED + amount=0.
- Единая классификация: `paid` / `granted` (админ) / `referral` / `trial`. Месяцы — только `amount_paid>0`. «С подпиской» = paid+granted+referral. «Рефералы» = best live `referral_bonus`.
- Дашборд: строка «Рефералы» + `subscriptions_referral`. Фильтры списка тем же best-kind.
- **Месяцы 1/2/3:** только если **самый долгий** живой план = купленный этот план. Старая покупка при выдаче unlimited / реф.бонусе в месяцы не попадает.

## Последние изменения (Подписки: фильтры как сортировка 2026-09-06)

- Старые чипы Все/Активна/Нет/Оплата → SortSelect: Все, С подпиской, 1/2/3 мес., Выданные, Без подписки, Оплата без подписки, Рефералы, Пробный период.
- API `GET /api/admin/subscriptions/users?filter=` расширен; `active` → alias `with_sub`.

## Последние изменения (Пользователи: сортировка неверифицированные 2026-09-06)

- В «Пользователи» добавлен пункт сортировки **Неверифицированные** (`is_verified=false` сверху). `UsersPage.tsx`.

## Последние изменения (Дашборд VK-хеши: сортировка со сбоями 2026-09-06)

- Убрано «Недавно в сети»; добавлено «Со сбоями сначала» (`fail_count` / неактивные слоты). `DashboardPage.tsx`.

## Последние изменения (Доступность: ложное «Проверка не выполнена» 2026-09-06)

- Симптом: бейдж «Доступно из РФ» + нормальный summary, но красная строка «Проверка не выполнена».
- Причина: POST `/availability/run` ждал весь прогон (~90 с); ответ обрывался, UI ставил error, хотя отчёт на сервере уже `ok`.
- Фикс: run в фоне (`started: true`), UI опрашивает GET пока `running`/сменится `last_run`. Текст «Проверка не выполнена» убран.

## Последние изменения (Доступность + аудит Улья + security skip 2026-09-06)

- **Ложные DPI «2 из 3»:** один timeout check-host при 3 нодах давал `asn_partial`. Фикс: `PARTIAL_MIN_FAILS=2` в `availability_classifier.py` (+ unit).
- **«Вмешательства»:** почти все были `AdminHostGuard` с `ip=172.18.0.1` (docker bridge при деплое/health), не взлом. Теперь security-события не пишутся с IP доверенных админ-устройств, `10.66.0.0/16`, `172.16.0.0/12`.
- **Аудит Улья (read-only):** SSH только с админ-ПК `79.111.*`; контейнеры штатные; `wdtt` active; fail2ban жив (1 бан); лишних ключей в `authorized_keys` нет; admin devices: живой «ПК» = тот же IP. Признаков компрометации нет.

## Последние изменения (Админ: подписи карточек Подписки/Онлайн 2026-09-06)

- «всего живых» → «всего»; в «Онлайн» подписи «все ноды…» / «максимум» внизу за разделителем, как у Подписок.
- Полоса и футер выровнены с карточкой Подписок (`flex` + `h-full` + `flex-1`), одна строка футера.

## Последние изменения (Админ: мобильные селекторы дашборда 2026-09-06)

- Селект ноды в «Системные ресурсы» и SortSelect у VK-хешей вылезали за карточку на телефоне (ряд title+select с `min-w` / без `min-w-0`).
- Фикс: на мобилке заголовок и контролы стопкой, `w-full min-w-0`; карточки `overflow-hidden`. `SortSelect`/`SearchInput` — `max-w-full min-w-0`.
- Правило на будущее: см. ниже «Admin UI: ПК + мобилка».

## Последние изменения (Админ: меню устройств на телефоне 2026-09-06)

- Попап «устройства админа» на мобиле уезжал вправо за экран (`absolute left-0` у иконки справа).
- Фикс: на `md-` — `fixed left-3 right-3 top-14` + backdrop; на ПК — как раньше dropdown вправо. `AdminDevicesMenu.tsx`.

## Последние изменения (Дашборд CPU: без фейковой частоты + модель сот 2026-09-06)

- Убрана «Онлайн: … MHz (≈ по загрузке)» — на KVM это не живой cpufreq, а оценка.
- Улей и соты показывают одинаково: номинал из model name + модель + N ядер. Cell-agent отдаёт `cpu_model` / `cpu_freq_base_mhz`.
- Селектор ноды в стиле SortSelect (иконка Server + ChevronDown).
- Соты подтянут новый cell-agent автоапгрейдом.

## Последние изменения (Дашборд: ресурсы Улей/Сота + CPU ядра 2026-09-06)

- В «Системные ресурсы» селект ноды (дефолт **Улей**); активные/draining соты подтягиваются сами из БД.
- API: `GET /api/admin/stats?node_id=queen|<uuid>`, `resource_nodes[]`. Сота — через cell-agent `/v1/status` (без диска).
- **CPU% корректен для 4 и 6 ядер:** это загрузка по всем ядрам 0–100% из `/proc/stat` (не «на ядро»). В UI: `CPU — X% · N ядер`.
- Файлы: `dashboard_node_stats.py`, `DashboardPage.tsx`. Деплой: `npm run build` + `deploy_stable.py`.
...
**Нерабочий код — откатить, потом другой подход (жёстко):**

Если правка не заработала (баг, «не коннектится», тест красный, пользователь говорит что сломано) — **сначала откатить эту реализацию** (`git restore` / `git revert` незакомиченного или последнего своего коммита), **потом** пробовать другой способ. **Запрещено** писать новый фикс поверх нерабочего: получается каша из двух реализаций. Один заход = один подход; не сработало — чистый откат, затем другой.

**Commit и push — только по явной команде пользователя.** Не коммитить и не пушить «заодно». Триггер «пуш» (см. Git workflow) = commit + push. Пока не сказали «пуш» / «закоммить» / «запушь» — код может быть в рабочей копии, в git его нет.
...
### Admin UI: ПК + мобилка (обязательно при любой новой фиче админки)

Админку смотрят и с телефона. Любой новый UI в `admin-ui/` должен **сразу** нормально выглядеть на узком экране (~360px), не только на десктопе. Инциденты 2026-09-06: меню устройств, селект ноды ресурсов, SortSelect у VK-хешей — всё «вылезало» за карточку.

**Правила:**

1. Шапка карточки с селектом/поиском: на мобилке **колонка** (`flex-col` → `sm:flex-row`), не `justify-between` в один ряд с `shrink-0` у заголовка.
2. Селекты/инпуты: `w-full min-w-0 max-w-full`; фиксированную ширину (`w-48`, `min-w-[…]`) только с `sm:` и выше.
3. Карточки/гриды: `min-w-0` (иначе flex/grid ребёнок не сжимается); при риске overflow — `overflow-hidden` на карточке.
4. Попапы/дропдауны у правого края экрана: на мобилке `fixed left-3 right-3` (или открытие влево), не `absolute left-0` от иконки справа. Пример: `AdminDevicesMenu.tsx`.
5. Переиспользовать `SortSelect` / `SearchInput` — там уже заложены мобильные ограничения; не плодить голые `<select>` с `min-w` в шапке.
6. Перед «готово»: глазами на ширине телефона (DevTools) или реальном телефоне — заголовки, селекты, таблицы (таблицы — `overflow-x-auto` обёртка, не раздувание страницы).
...
Масштабирование VPN: **Улей** (главный VPS) + **соты** (дополнительные VPS). Пользователь **сам выбирает** Сервер 1/2/3 в клиенте; API не перекидывает людей на соту из‑за «переполнения» Улья. Строка «онлайн лимит» в карточке Улья — оценка ёмкости для UI/автобаланса, **не** отсечка connect.
...
### Агент доступности — детекция блокировок DPI/ТСПУ (AI-агент)

Отвечает: **видят ли клиенты из РФ наши серверы, а если нет — как именно заблокировали и что делать.**
Фоновый агент под лидером, интервал 15 мин. Пробы **только читают**: не рестартит `wdtt`, не трогает peer'ы,
не переключает DNAT. Полный runbook: `backend/AVAILABILITY.md`.

| Компонент | Путь |
|-----------|------|
| Модель данных | `ai/availability_model.py` |
| База знаний методов блокировок + решения | `ai/availability_knowledge.py` |
| Классификатор (чистые функции, без сети/БД) | `ai/availability_classifier.py` |
| Пробы (локально / `check-host.net` / соты) | `ai/availability_probes.py` |
| Фоновый агент | `ai/availability_agent.py` (в `agent_specs` в `main.py`) |
| CLI по SSH | `ai/availability_cli.py` |
| Хранилище отчётов и телеметрии | `app/services/availability_store.py` |
| Админка | Улей → «Доступность и блокировки», `HiveAvailabilityPanel.tsx` |
| Тесты | `scripts/test_availability_unit.py` (28 unit-тестов, без сети/БД) |

**Тихий режим (обязательное свойство):** журнал «Инциденты» общий для Улья, поэтому агент пишет туда
только подтверждённое двумя циклами, не чаще раза в 12 ч по одной проблеме, ≤2 записи за цикл;
ручной запуск не пишет вообще. Логика — `plan_incidents`, покрыта тестами.

**Ресурсы:** интервал 30 мин, пропуск цикла при CPU ≥85 % / RAM ≥92 %, снаружи ≤3 узла и ≤10 запросов
за цикл, хранение 300 отчётов и 48 ч клиентских репортов. Цели — только Улей и соты (proxy-ноды
ведёт `proxy_health_loop`).

**Точки наблюдения:** локально с Улья; российские ноды `check-host.net` (взгляд из-за ТСПУ); контроль вне РФ
(если отказ виден везде — это не блокировка); соты через `POST /v1/net-probe` на cell-agent; клиентская
телеметрия `POST /api/vpn/reachability-report` (стадия отказа, тип сети, оператор).

**Клиентская телеметрия — зачем, если сервер недоступен.** Она не про полный блок (его видят
российские ноды), а про то, чего внешние пробы не видят и где API как раз доступен: срез UDP
(`check-host` не умеет честно проверить наш UDP), DPI по отпечатку потока (`tunnel_dead` с возрастом
туннеля), реальность абонента мобильного оператора. Когда прямой путь до Улья закрыт, репорт идёт
через соту (`vpn/*` в белом списке публичного failover `:9100`, другой IP и порт — сота проксирует
на Улей) или bootstrap-туннель по VK-хешу. Не дошло — клиент держит в очереди и досылает с полем
`age_sec`: сервер метит отказ временем **самого сбоя**, поэтому отложенная пачка не создаёт ложную
блокировку в текущем окне; старше 48 ч отбрасывается. Очереди: `pc/src/renderer/reachabilityReporter.ts`,
`android/.../vpn/ReachabilityReporter.kt` — не привязаны к каналу туннеля.

**Различает:** `ip_blackhole`, `port_block`, `udp_block`, `sni_block`, `protocol_fingerprint`, `dns_poisoning`,
`rst_injection`, `http_stub`, `throttling`, `asn_partial`, `mobile_shutdown`, `route_loss`, `service_down`,
`no_vantage`. У каждого вывода — доказательства, уверенность и **готовое решение** с командами.

**Если админка недоступна** — тот же отчёт по SSH:
...
Таблицы: `availability_reports`, `availability_client_reports`. Настройки — `AVAILABILITY_*` в `config.py`,
перезаписываются из админки (`app_settings`); выключить агент можно галочкой, без деплоя.
...
|--------------------|----------|
| «пуш» | commit + push из нужной папки; перед этим снять мусор (CRLF/BOM/шум) во всех ветках, не оставлять висящий `git status` |
...
**Пуш только по требованию:** commit и `git push` **запрещены**, пока пользователь не сказал «пуш» (или явно «закоммить» / «запушь»). Не пушить после фикса «чтобы не потерялось».

**Перед пушем — убрать мусор из рабочей копии (жёстко):** не оставлять висящие «изменения» без смысла. Проверить `git status` / `git diff` во **всех** репозиториях (`backend`, `pc`, `android`, `ios`), не только в том, куда идёт коммит.

Считать мусором и **не коммитить**:
- только переводы строк CRLF/LF (`LF will be replaced by CRLF`, diff пустой по смыслу);
- BOM в начале файла (`﻿package` / `﻿import`);
- случайный whitespace, `.DS_Store`, `__pycache__`, логи, `dist`/сборки, временные `_tmp_*`.

Такое **исправить или откатить** (`git restore <файл>`), чтобы `git status` был чистый кроме настоящей правки. Настоящий код в коммит; шум не должен висеть после пуша.

**Нерабочая правка:** не наслаивать второй фикс. Откатить сломанный код, затем другой подход. См. «Ключевые решения» → откат нерабочего кода.
...
- **Дашборд меньше Улья:** 2026-08-27 `/api/admin/stats` взял `is_connected` из БД. Шапка Улья = сумма WG live. Дашборд снова то же число (кэш 20 с).
- **Соты «мало людей»:** не кап 20 на connect. Пользователь сам выбирает Сервер 1/2/3; `/config` и `preferred_server` не смотрят `max_online`. Строка «онлайн лимит» в карточке — оценка (Сота с 1 Гбит масштабируется от эталона 10 Гбит ≈ 10%), не запрет. Жёсткого WG-peer cap нет.
- Автоперекидывание с Улья на соты при «переполнении» — не наш подход; ошибочный фикс spill откатил.
- **Прод 2026-08-29:** `deploy_stable.py` — health 200 (~39 мс), `wdtt` active, DNAT `10.66.66.1:8000` на API, `queen_wg_kick_20s=0`. Дашборд онлайн = шапка Улья. Админку открыть с Ctrl+F5. Пуш `origin/main`.
- Перед любым «пуш»: снять CRLF/BOM/пустой diff (`git restore`), не оставлять шум в `git status` (Git workflow).

### 2026-08-29 — Хард-ресет YouTube-экспериментов

- **android** `1b34d36` на `2704284`: только сессии на LTE и merge списка устройств. YouTube/IPv6/DNS/rotate_wg сняты с ветки.
- **backend** `main` = `a810026` (сортировка онлайн в админке). `rotate_wg` с истории снят.

### 2026-08-28 — Админка: сортировка пользователей

- Дашборд (список VK-хешей по пользователям) и страница «Пользователи»: селект сортировки рядом с поиском — онлайн, алфавит А→Я / Я→А, дата регистрации (новые/старые), недавно в сети (дашборд), подписка (страница пользователей). Выбор запоминается в localStorage.
- API: `GET /api/admin/stats` → `vk_users[].created_at`; `GET /api/admin/users` → опциональные `is_online` / `online_devices`. Старые клиенты не затронуты (только админка).
- Прод: `deploy_stable.py` — health 200 (~60 мс), wdtt active, DNAT на API, kick/20с = 0. Обновить админку с Ctrl+F5.
...
- Фильтр «Активна» брал только 500 самых новых аккаунтов и уже среди них искал живые подписки. Старые аккаунты с ручным unlimited были в «Все», но не в «Активна».
- Теперь фильтр идёт SQL-ом по всей базе; unlimited (дальний expires_at) сверху списка.
...
- Фикс: опциональное `age_sec` (сколько репорт пролежал в очереди) → сервер метит запись временем **самого отказа** (`now − age_sec`); старше 48 ч — `accepted: false, detail: stale report`. Политика вынесена чистой функцией `client_report_age_sec` в `ai/availability_model.py` (тестируется без БД).
- Клиенты: `pc/src/renderer/reachabilityReporter.ts` и `android/.../vpn/ReachabilityReporter.kt` — debounce 5 мин на стадию, очередь ≤40, экспоненциальный backoff, выгрузка после поднятия туннеля и на Wi‑Fi tick. Хуки: ошибка VPN при подключении → `handshake`; смерть живого туннеля (не по кнопке) → `tunnel_dead` с возрастом туннеля.
- Очереди **не** привязаны к `isTunnelApiActive()`: у `hashFailureReporter` при основном VPN этот флаг всегда false, из-за чего его очередь не выгружалась. Здесь просто пробуем POST, а маршрут tunnel → public → соты выбирает общий слой API.
- Пути доставки при блокировке: путь `vpn/*` в белом списке публичного failover соты (`:9100`, другой IP и порт — сота проксирует на Улей) и bootstrap-туннель по VK-хешу. Полный blackhole телеметрия не покрывает, но его видят российские ноды `check-host`.
- Android `compileDebugKotlin` — успешно; тестов стало 33 (было 28). Правки на прод **не деплоились**.

### 2026-08-23 — Агент доступности на проде: тихий журнал и лимит по ресурсам

- Задеплоено `deploy_stable.py`. После деплоя: `wdtt` **active**, DNAT туннеля на IP Улья `:8000`, health 200, **7** фоновых агентов, 92 устройства онлайн. Живой прогон: Улей + Сота 1 + Сота 2 доступны из РФ (3 ноды `check-host`), 30 с, 8 внешних проверок, проблем нет. `backend-api-1`: CPU ~5 %, RAM 248 МБ.
- **Журнал инцидентов не забивается** (`plan_incidents`, чистая функция + тесты): только `critical`/`error`, проблема должна подтвердиться **2 циклами подряд**, повтор о том же — не чаще **12 ч**, максимум **2 записи за цикл**, решённая проблема забывается, **ручной** запуск (кнопка и `--run`) в журнал не пишет вообще. О своей поломке агент сообщает после 3 фейлов и не чаще раза в 12 ч. Факт: в `hive_incidents` от `hive.availability` — 0 записей.
- **Ресурсы**: интервал 30 мин (минимальный сон 5 мин), перед прогоном проверка нагрузки — при CPU ≥85 % или RAM ≥92 % цикл **пропускается**; снаружи проверяются первые 3 узла (Улей первым) и ≤10 запросов за цикл; хранение 300 отчётов, клиентские репорты 48 ч и ≤20 000 строк.
- **Не ломает другие агенты**: импорт `availability_agent` вынесен в свой `try/except` в `main.py` (раньше падение его импорта лишило бы прод VK-агента, балансировки и обслуживания сот), DDL его таблиц — отдельной транзакцией после основной схемы.
- Proxy-ноды убраны из целей: ими занимается `proxy_health_loop`, а их SOCKS закрыт для Улья — иначе агент писал бы ложное «сервис не отвечает». Непроверенная снаружи сота больше не гасит общий статус до `unknown`.
- Тестов стало 28 (было 22): добавлены проверки подавления инцидентов, окна тишины, лимита за цикл и того, что деградация в журнал не идёт.

### 2026-08-23 — Новый AI-агент: доступность серверов и детекция блокировок DPI/ТСПУ

- Задача: понимать, видят ли клиенты из РФ наши серверы, **как именно** нас заблокировали (IP / порт / SNI / UDP / DNS / отпечаток протокола) и **что делать** — решение в самом отчёте.
- Три источника правды сравниваются между собой: локальные пробы с Улья, российские ноды `check-host.net` (взгляд из-за ТСПУ), контроль вне РФ. Плюс пробы со сот (`/v1/net-probe`) и клиентская телеметрия (`POST /api/vpn/reachability-report`: стадия отказа, транспорт, тип сети, оператор).
- Логика разнесена: `availability_probes` (сеть) → `availability_classifier` (чистые функции, 22 unit-теста без сети/БД) → `availability_knowledge` (методы блокировок + фиксы) → `availability_agent` (фоновый цикл под лидером, 15 мин).
- Ключ к различению: TCP открыт, а TLS с нашим SNI — нет → блок имени; UDP мёртв, TCP жив → срез UDP; всё молчит из РФ, но живо локально и вне РФ → blackhole IP; reset вместо таймаута → RST-инъекция; RTT из РФ втрое выше → шейпинг. 3xx от нашего nginx считается успехом (ответ дошёл), а не блокировкой.
- Fail-safe по `vpn-safety`: агент только читает — ни рестартов `wdtt`, ни переключения DNAT, ни снятия peer'ов; на живых клиентов не влияет (клиентский репорт опционален, поля новые).
- Админка: Улей → «Доступность и блокировки (DPI / ТСПУ)» — статус, проблемы с решениями и командами, детали по каналам, справочник методов. Настройки (вкл/выкл, интервал, число нод) — из UI, без деплоя.
- Если админка недоступна: `docker exec backend-api-1 python -m ai.availability_cli [--run|--history N|--clients|--knowledge|--json]`. Runbook — `backend/AVAILABILITY.md`.
- Живая проверка парсера на реальном `check-host.net`: 58 нод, RU-ноды найдены, TCP/ping/DNS/HTTP разбираются корректно. На прод **не деплоилось**.
...
- Фикс: пустая БД = пустой список; «Очистить» ставит `incidents_cleared_at` и не даёт вставить события старше метки. Кнопка с press-state.
- Прод: API + admin-ui/dist, **wdtt не рестартим.**
...
- Решение для всех клиентов: iptables цепочка `SILENT_DENY` по точному inner-IP `devices[device_id].ip`. GETCONF может поднимать peer — FORWARD всё равно DROP. `10.66.66.1` и `boot:` не трогаем. Чужие extras не угадываем.
- Откат unit сот (`backend-url` снят, daemon-reload). **wdtt не рестартили** (Улей PID с 1.08). API+cell-agent задеплоены.

### 2026-08-19 — Отзыв + тумблер: leftover GETCONF extra

- Сценарий «выкл тумблер → отзыв → вкл»: клиент поднимал **старый GETCONF extra** (другой ключ/IP, чем в БД). Kick бил только ключ из `devices` — трафик жил.
- Защёлка disconnect 90 с игнорировала `online=true` после повторного включения, kick не стартовал. При «Нет» защёлка больше не прячет сессию.
- На выкл запоминаем leftover extra; на отзыве снимаем его; при включении — extra, который «воскрес» со stale handshake. Чужие extras не трогаем.
- Прод: `deploy_stable.py`, health `ok`, tunnel OK, `wdtt` active, **wdtt не рестартили**.

### 2026-08-19 — Сервер сразу рвёт VPN, если подписки нет

- В админке «Нет» → dataplane режется сразу: известный WG-ключ, **все** peer’ы с точным IP устройства, GETCONF extra только этого устройства (тот же IP / единственный новый extra на ноде). Чужие extras не трогаем (инцидент 16.08).
- Повторный тумблер: keepalive `/internal/online` с `vpn_allowed=false` снова снимает peer; sweeper ~10 с держит watch 25 мин. Выключение теста без платной подписки тоже kick.
- Сота: каждый цикл агента снимает peer’ы с `vpn_allowed=false` по ключу и IP. **wdtt не рестартили.**
- Прод: `deploy_stable.py` — health `ok`, tunnel `10.66.66.1:8000` OK, `wdtt` active.
...
### 2026-08-19 — LTE: только bootstrap на старте; страховка на сервере

- Мобильный интернет: сверка подписки **только** стартовый bootstrap (`/me` + ephemeral) — он и выдаёт, и отзывает. С тумблера GETCONF-гейт, ожидание GETCONF и HTTP overlay убраны: main VPN из кеша как 1.0.160.
- Если приложение не закрывают и VPN всегда включён — bootstrap не повторится. Страховка: API каждые ~10 с снимает **известный** WG-peer у устройств без подписки и ставит `vpn_allowed=false` в manifest соты.
- Fail-open: админ, глобальный тест, живой `plan=test`, оплата/trial — **не** режем (сейчас у всех бесплатный тест). Чужие GETCONF extras не трогаем (инцидент 16.08).
- Задеплоено `deploy_stable.py` 19.08.2026 ~20:18: health `ok`, tunnel `10.66.66.1:8000` OK, **wdtt не рестартили**. Debug APK: `android/SilentVPN-debug.apk`.
- Версия 1.0.161.

### 2026-08-19 — Тумблер как PC: интернет только из живого GETCONF/DTLS

- На LTE + белые списки публичный HTTP к API недоступен. Сверка подписки **в обе стороны** только GETCONF по DTLS: выдали WG → остаёмся; `DENIED:` при старте и каждые 30 с → гасим и «Оформить».
- Как PC: кеш конфига нужен лишь чтобы запустить libclient (хеши/пароль). **WG-кеш интернет не поднимает** — иначе без подписки туннель уже живой.
- Старый `wg-turn.conf` не считается GETCONF. Overlay `/me` после main VPN на LTE не трогаем (рвёт DTLS). Старт/bootstrap без изменений.
- Версия 1.0.161. Debug: `android/SilentVPN-debug.apk`.

### 2026-08-19 — Тумблер: два канала (кеш + живой GETCONF/DTLS)

- Баг: `wg-turn.conf` со вчера считался GETCONF → VPN включался **без** живой сверки, в том числе без подписки.
- Как раньше: канал 1 — быстрый WG из кеша, только если локально подписка есть (`fastWgCache`). Канал 2 — **живой** GETCONF по DTLS (файл строго после старта libclient / stdout «Конфиг получен»). `DENIED:` / FATAL_AUTH гасит VPN и «Оформить».
- Без подписки кеш не поднимаем; `CONNECTED` только после живого GETCONF. Старый файл больше не маскируется под GETCONF.
- Старт приложения / bootstrap не трогали. Версия 1.0.161. Debug: `android/SilentVPN-debug.apk`.

### 2026-08-19 — Старт: /me + WG-кеш; тумблер: кеш + GETCONF

- На старте `/me` в обе стороны (есть подписка → «Оплачено», нет → «Оформить»). Prefetch после `/me` обязан сохранить WG-конфиг (на LTE через ephemeral). Это не трогает splash.
- Тумблер **не** ходит в публичный `/vpn/config` и не делает второй bootstrap. Схема: кеш со старта → main VPN → GETCONF по DTLS. Выдали конфиг → остаёмся; `DENIED:` → гасим и «Оформить».
- Если тумблер нажали, пока стартовый prefetch ещё качает ключи — **ждём его**, не отменяем. Иначе на LTE подписка уже есть, а конфига нет, и тумблеру нечем достучаться до сервера.
- UI «до null» убран: дата только если `expires_at` есть. Версия 1.0.161.

### 2026-08-19 — Тумблер: GETCONF дал подписку — не гасить stale «нет»

- Если в приложении «Оформить», а на сервере подписка есть: GETCONF включает VPN и показывает оплату.
- Сразу после этого старый кеш/`/me` «нет» больше **не** отключает туннель. Гасит только `DENIED:` / FATAL_AUTH.
- Живой `/me` с `is_active=true` по-прежнему подставляет дату. Версия 1.0.161.

### 2026-08-19 — Канон: старт bootstrap, тумблер = GETCONF/DTLS

- **Старт приложения:** bootstrap как сейчас. `/me` + конфиг: подписка на сервере есть → «Оплачено»; нет → «Оформить». Splash не ломаем.
- **Тумблер:** быстрый старт main VPN из кеша, **без** второго bootstrap и **без** HTTP overlay (главный интернет не рвём). Второй канал сверки — только **GETCONF по DTLS**. Значок может один раз мигнуть — это перепроверка.
- GETCONF выдал WG → остаёмся. `DENIED:` → гасим и «Оформить».
- Версия 1.0.161.

### 2026-08-19 — Android: bootstrap = отзыв, тумблер = только DTLS GETCONF

- **Главный экран:** bootstrap/`/me` проверяет отзыв. Нет подписки → paywall. Splash его не ждёт.
- **Тумблер:** не bootstrap и не `/vpn/config`. Только кеш + main VPN. WG поднимается из GETCONF. Выдали подписку → остаёмся; DENIED → гасим.
- Prefetch на тумблере **не** отменяется: если кеша ещё нет, тумблер ждёт стартовый bootstrap, затем GETCONF.

### 2026-08-19 — LTE тумблер: GETCONF по DTLS, без overlay /me

- На мобильном после тумблера **не** вызываем `/me` через overlay — он рвал DTLS, GETCONF не успевал, сразу «нет подписки».
- Доступ на LTE решает GETCONF: выдали — сессия живёт, UI как со старта; `DENIED:` — оформить. Wi‑Fi по-прежнему обновляет дату из `/me`.

### 2026-08-19 — Тумблер: UI из живого /me, не заглушка GETCONF

- GETCONF больше **не** ставит локально «оплачено». После тумблера UI = живой `/me`: есть подписка → «Оплачено до ДД.ММ.ГГГГ», нет → «Оформить».
- Стиль строки как раньше. Stale `client_sync` не перебивает уже полученный `/me`. Старт приложения не трогали.

### 2026-08-19 — Android: GETCONF с живой подпиской не должен писать «нет»

- Локальный флаг отзыва больше не побеждает DTLS: если GETCONF прошёл — доступ есть, флаг снимаем, тумблер остаётся.
- «Нет подписки» только при `DENIED:` / FATAL_AUTH. Таймаут без DENIED — обычная ошибка подключения, не paywall.
- Bootstrap по-прежнему после splash на главном. Версия 1.0.161.

### 2026-08-19 — Подписка bootstrap в обе стороны + онлайн = WG live

- **Android/PC старт:** `/me` и `/vpn/config` (bootstrap на LTE) включают UI если подписка есть и выключают если нет. Старый флаг `vpn_access_denied` больше не блокирует живой 200. Prefetch после тумблера не затирает GETCONF.
- **Улей/дашборд:** большое число «онлайн» = сумма `wg_peers_live_3m` по Улью и сотам (то, что в строке live). Лимит/spill по-прежнему из БД. Тест: `scripts/test_hive_slots_unit.py`.

### 2026-08-19 — Android: bootstrap на старте, но не на boot-анимации

- Bootstrap на запуске **вернул**: он ловит отзыв подписки (402 через DTLS), особенно на LTE.
- Splash его **не ждёт**: ~0.9 с анимация → сразу главный экран с тумблером. Prefetch/bootstrap дорабатывает уже там и гаснет после данных.
- Если пользователь нажал тумблер во время ephemeral — bootstrap не рвёт main VPN.
- Флаг `vpn_access_denied` и запрет воскрешать «оплачено» из `client_sync` остаются. Версия 1.0.161.

### 2026-08-18 — Android: splash без bootstrap + отзыв не воскресает из кеша

- Splash больше не поднимает ephemeral bootstrap и не ждёт 18 с boot-анимации: только public `/me` + `/vpn/config` (макс. 2.5 с).
- После отзыва флаг `vpn_access_denied` переживает перезапуск. Старый `client_sync.profile` («оплачено», дни) больше не затирает отзыв.
- Тумблер при отзыве не считает кеш подпиской. Версия 1.0.161.

### 2026-08-18 — Android: тумблер без подписки не остаётся онлайн по WG-кешу

- Splash по-прежнему ловит 402 и ставит «Оформить». WG-кеш не стираем (иначе тумблер без bootstrap).
- Если локально подписки нет: main VPN стартует для GETCONF, но **`fastWgCache=false`**, API_CACHE запрещён. `CONNECTED` только после GETCONF.
- `DENIED:` / `FATAL_AUTH` / таймаут GETCONF → гасим VPN и paywall. Если GETCONF прошёл — подписку выдали, остаёмся онлайн.
- Версия 1.0.161. Debug APK: `android/SilentVPN-debug.apk`.

### 2026-08-18 — Android LTE: конфиг на splash, тумблер без мигания

- **Старт:** быстрый bootstrap — отозвана ли подписка. 402 → кнопка «Оформить», локальный `is_active=false`. WG-кеш **не** стираем.
- **Тумблер:** без второго bootstrap/overlay. Сразу main VPN из кеша; проверка — GETCONF по DTLS (быстрый миг: `DENIED:` гасит, выдали подписку — остаётся). Улей или сота одинаково.
- Смена сервера **не** стирает WG-кеш (иначе тумблер без bootstrap писал «нет конфига»). GETCONF подставит ключи новой ноды.
- Splash не режет prefetch (`viewModelScope`). Версия 1.0.161.

### 2026-08-18 — Онлайн: дашборд = шапка Улья = сумма карточек

- С 2026-08-19 большое число = сумма `wg_peers_live_3m` (live), не `is_connected` в БД.
- Ранее: каждое `is_connected` устройство на одной ноде; WG live в карточку не входил.
