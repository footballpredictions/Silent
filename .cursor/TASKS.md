...
- [ ] **olcrtc 2.0** — WDTT-совместимый headless движок Телемост/WB на отдельной соте (план `.cursor/PLAN_OLCRTC2.md`, `backend/olcrtc2/`). Phase 0–2 ✅ (Telemost carrier + `olcrtc2-srv` на Соте 1 `87.58.213.193`). Дальше Phase 3 WB. **Не деплоить на Улей рядом с wdtt.** Нужен `OLCRTC2_ROOM`+`KEY` для старта unit.
