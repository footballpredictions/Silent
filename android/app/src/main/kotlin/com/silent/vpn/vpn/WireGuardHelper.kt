...
class WireGuardHelper(context: Context) {
    ...
    suspend fun startTunnel(
        ...
        withContext(Dispatchers.IO) {
            ...
            val routeAsBootstrap = isBootstrap || apiOverlayMode
            runCatching {
                val excluded = resolveExcludedAppPackages(appContext, routeAsBootstrap)
                if (excluded.isNotEmpty()) {
                    ifaceBuilder.excludeApplications(excluded)
                    DebugLog.i(TAG, "App exclusions: ${excluded.size} (bootstrap=$routeAsBootstrap overlay=$apiOverlayMode)")
                }
            }
