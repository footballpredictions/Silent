...
- [ ] Деплой `ai/availability_classifier.py` на Улей: с этой сети SSH/HTTPS Улья таймаут; соты :22/:9100 живы, пароль Улья на соты не подходит. Нужен VPN/другая сеть → `deploy_stable.py`

### Улей: публичный TCP из РФ (2026-09-15)

Не падение сервиса. Из РФ/дома **весь TCP** к `132.243.234.162` (22/80/443/8443) — timeout;
ICMP частичный, UDP WG жив (онлайн есть), соты `:22`/`:9100` живы. Не SNI (TCP даже не
открывается). Похоже на **IP в списке / whitelist провайдера**, не классический DPI по домену.

- [ ] Не чинить 443 на том же IP (DNAT/другой порт на том же адресе бесполезен)
- [x] Сота `:9100` и standby `10.66.66.1` сами ходят на Улей `IP:8000` (не nip.io) и отдают вход/оплату/подписку/sync; снимок только если Улей с соты реально мёртв. Сота 1/2 залиты, Сота 3 агент не рестартнулся
...
- [ ] Новый IP Улья — только если снова нужен публичный HTTPS из РФ; другой ASN, не nip.io, UDP WG можно оставить как есть (wdtt не рестартить на живых)
