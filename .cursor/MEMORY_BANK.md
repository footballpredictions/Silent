...
- Одна формула: каждое `is_connected` устройство ровно на одной ноде (`assign_online_to_cell_id`: pin server2+ → сота, иначе `cell_id`, иначе Улей).
- Дашборд «Онлайн» = шапка Улья «Онлайн всего» = Улей + Сота 1 + Сота 2. WG live / olcrtc sticky в большое число **не входят** (строка WG — диагностика).
- Раньше расходилось: дашборд 88, шапка 113, карточка Улья 95 (мусор GETCONF + пересечение default `server1`).
- Тест: `scripts/test_hive_slots_unit.py`. Деплой: `deploy_stable.py` (wdtt не трогали).
