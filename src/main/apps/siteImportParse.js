 ...
 * Без Node DNS — можно дергать из renderer и unit-тестов.
 */
...
function walkJsonNode(node, addRaw) {
  if (node == null) return
...
function extractFromPlainText(text, addRaw) {
  try {
...
function extractRulesFromImportContent(content) {
  const found = new Map()
  const addRaw = (raw) => {
    const n = normalizeRuleInput(stripQuotes(raw))
    if (!n) return
...
function mergeImportRules(existing, imported, maxRules = MAX_RULES) {
