...
1. **Unit (backend, `backend/tests/` или рядом с существующими smoke):**
   - `get_wallets()`: 2 кошелька / 5 кошельков / дырки в нумерации (`WALLET_1`, `WALLET_4`) / 0 кошельков
   - подпись: валидная / невалидная / секрет-fallback без `SECRET_N`
   - notify: amount mismatch → failed; codepro=true → игнор; повтор operation_id → идемпотентно; label чужого формата → игнор
   - гонка: два конкурентных notify по одному label → одна подписка (asyncio.gather на тестовой сессии)
2. **Smoke на проде (по образцу `scripts/smoke_referral.py` → новый `scripts/smoke_payments.py`):** `/plans`, `/init` (возвращает url с receiver из списка кошельков + label), `/status/{label}` = pending, ручная симуляция notify с тестовым секретом на dev-платеже.
3. **Ручной e2e:** реальный платёж на минимальный тариф с каждого кошелька (проверить распределение: сделать 5–10 init и убедиться, что receiver в url меняется). Проверить активацию без каких-либо действий в клиенте.
4. **Клиенты:** PC debug-сборка + Android debug APK — экран ожидания, «Оплата получена», авто-reconnect.
...
10. Владелец: настроить HTTP-уведомления + секреты обоих кошельков (§9), «Протестировать» из кабинета
11. Реальный e2e-платёж (§11.3)
12. Обновить `.cursor/APIS.md` (payments section: status endpoint, per-wallet секреты, wallet env-схема) и «Последние изменения» в `MEMORY_BANK.md`; закрыть задачу в TASKS.md про документацию YuMoney webhook flow
13. Push: `backend` → `main`, `pc` → `pc`, `android` → `android` (по явной команде пользователя)

**Не делать:** активацию из клиента; общий секрет на все кошельки; WebView для оплаты; новые deploy-скрипты; хардкод текстов оплаты в клиентах.
