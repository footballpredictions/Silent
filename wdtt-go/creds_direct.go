...
// getLanIPv4 — Wi‑Fi/Ethernet (192.168.x), не WG и не vEthernet/WSL.
...
// vkDirectResolver — DNS напрямую на Яндекс, с привязкой к LAN (если найден).
...
func vkDirectResolver() *net.Resolver {
	...
	return &net.Resolver{
		...
		Dial: func(ctx context.Context, network, address string) (net.Conn, error) {
			...
			return dialViaLan(ctx, network, address, 3*time.Second)
